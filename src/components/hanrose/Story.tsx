...
export const Story = () => {
  ...
  const images = s?.about_images ?? [];
  const linkUrl = s?.about_link_url ?? null;
        ...
        <div className="relative order-2 lg:order-1">
          ...
          {images.length > 0 ? (
            <SimpleSlider
              images={images}
              linkUrl={linkUrl}
              className="aspect-[4/5] max-w-md mx-auto rounded-3xl overflow-hidden shadow-elegant"
            />
          ) : (
            <Placeholder label="Foto Owner / Brand Story" variant="cream" icon="heart" className="aspect-[4/5] max-w-md mx-auto shadow-elegant" />
          )}
        </div>
